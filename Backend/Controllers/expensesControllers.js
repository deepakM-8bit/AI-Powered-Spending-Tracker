...
export const addExpenses = async (req, res) => {
  ...
  const safeRecurring =
    typeof recurring === "string" && recurring.trim()
      ? recurring.trim()
      : "none";

  try {
    const result = await pool.query(
      "INSERT INTO expenses (title,amount,category,date,recurring,note,user_id) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
...
export const updateExpenses = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE expenses SET title=$1,amount=$2,category=$3,date=$4,recurring=$5,note=$6 WHERE id=$7 AND user_id=$8 RETURNING *",
